...
//x	return [[[NSString alloc] initWithBytes:[data bytes] length:[data length] encoding:[self responseEncoding]] autorelease];
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		...
		if (!tempUserAgentString) {
            ...
            if(self.tag >= 0x80001212) {
                tempUserAgentString = [tempUserAgentString stringByReplacingCharactersInRange:NSMakeRange(tempUserAgentString.length-1, 1) withString:[NSString stringWithFormat:@":%ld)", self.tag - 0x80001212 + 1]];
            }
		}
		if (tempUserAgentString) {
			[self addRequestHeader:@"User-Agent" value:tempUserAgentString];
//            NSLog(@"User-Agent : %@", tempUserAgentString);
		}
	}
...
- (void)incrementDownloadSizeByForce:(long long)length
{
    [ASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeByForce:) onTarget:&downloadProgressDelegate withObject:self amount:&length callerToRetain:self];
}
	...
	if (![self willRedirect]) {
		...
		if (cLength) {
			...
			if ([self mainRequest] && [self allowCompressedResponse] && length == 20 && [self showAccurateProgress] && [self shouldResetDownloadProgress]) {
                ...
                [theRequest incrementDownloadSizeByForce:[theRequest contentLength]+[theRequest partialDownloadSize]];
			}
...
- (void)handleBytesAvailable
{
	...
	[ASIHTTPRequest performSelector:@selector(request:didReceiveBytesByForce:) onTarget:&downloadProgressDelegate withObject:self amount:&bytesRead callerToRetain:self];
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		// Are we downloading to a file?
            ...
            if(self.downloadDataHearderEncryptEnable) {
                [self encryptDownloadDataHeader:buffer length:bytesRead];
            }
            else {
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	[newRequest setDownloadDataHearderEncryptEnable:[self downloadDataHearderEncryptEnable]];
...
#if (ASI_AGENT_STRING_APP_NAME_CONSISTOF_ENABLE == TRUE)
			NSString *appName = [bundle objectForInfoDictionaryKey:@"CFBundleDisplayName"];
			if (!appName) {
				appName = [bundle objectForInfoDictionaryKey:@"CFBundleName"];
			}

//x			NSData *latin1Data = [appName dataUsingEncoding:NSUTF8StringEncoding];
//x			appName = [[[NSString alloc] initWithData:latin1Data encoding:NSISOLatin1StringEncoding] autorelease];
...
- (void)encryptDownloadDataHeader:(UInt8*)buffer length:(int)length
{
    static NSMutableData* tempBuffer = nil;
    if(!tempBuffer)
        tempBuffer = [[NSMutableData alloc] init];

    int encryptByte = kDownloadContentEncryptBlockMAxSize;
    if(self.contentLength < encryptByte)
        encryptByte = self.contentLength;

    [tempBuffer appendData:[NSData dataWithBytes:buffer length:length]];

    if(tempBuffer.length >= encryptByte) {
        
        [self setDownloadDataHearderEncryptEnable:NO];

        NSMutableData* inputData = [NSMutableData dataWithData:[tempBuffer subdataWithRange:NSMakeRange(0, encryptByte)]];
        [ClaEncryptUtil encryptData:inputData keyString:self.EncryptKeyString];
        [tempBuffer replaceBytesInRange:NSMakeRange(0, encryptByte) withBytes:inputData.bytes];
        inputData = nil;

        [[self fileDownloadOutputStream] write:tempBuffer.bytes maxLength:tempBuffer.length];
...
@synthesize downloadDataHearderEncryptEnable;
@synthesize EncryptKeyString;
